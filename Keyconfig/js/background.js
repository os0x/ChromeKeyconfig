...
function checkVersion(ver, base_ver) {
  var vers = ver.split('.').map(function(v){return parseInt(v,10);});
  var base_vers = base_ver.split('.').map(function(v){return parseInt(v,10);});
  ...
  action_names:[
    ...
    },
    {
      ...
      actions:[
        ...
        {name:"fastback",args:[]},
        {name:"forward",args:[]},
        {name:"go to #1", args:[{type:'URL'},{type:'NAME'}]},
...
function LDRize(){
  ...
  function UpdateSiteinfo(){
    ...
    //var url = 'http://wedata.net/databases/LDRize/items.json';
    var xhr = new XMLHttpRequest();
    xhr.onload = function(){
      try {
